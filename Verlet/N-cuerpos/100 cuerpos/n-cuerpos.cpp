...
int main()
{
    ...
    const long int Niter = 1.825e4; //1825000; // Numero de iteraciones (5000 anios).  
    const int out_cada   = 1;//1000;    // Out_cada iteraciones.
    
    n_cuerpos = 3; // Numero de cuerpos.
    ...
    // Archivos de salida.
    ofstream of_posicion("3-posicion.dat", ios::out);
    // ofstream of_velocidad("velocidad.dat", ios::out);
    ofstream of_energia("3-energia.dat", ios::out);
    ofstream of_momentumLineal("3-momLineal.dat", ios::out);
    ofstream of_momentumAngular("3-momAngular.dat", ios::out);
