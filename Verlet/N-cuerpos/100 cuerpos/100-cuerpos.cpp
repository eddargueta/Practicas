...
//uniform_real_distribution<long double> random_0_a_2(0.0, 2.0);
...
{
    // E, P, L de todo el sistema. 
    E, P, L = 0.0;
    
    double V      = 0.0; // Potencial.
    double Vc     = 0.0; // Potencial cruzado.
    double mtotal = 0.0; // Masa del sistema.
    double Eint   = 0.0; // Energia interna.
    double Rcm    = 0.0; // Posicion centro de masas.
    double Pcm    = 0.0; // Momento lineal total.
    double Lint   = 0.0; // Momento ang. interno.

    // Pcm = 0.0;
    for (int j = 0; j < n_cuerpos; j++)
    {
        double sqrt_x2_mas_y2   = sqrt(pow(xp[j], 2) + pow(yp[j], 2));
        double vx2_mas_vy2      = pow(vx[j], 2) + pow(vy[j], 2);
        double sqrt_vx2_mas_vy2 = sqrt(vx2_mas_vy2);
        double scl_xpvx_mas_ypvy = xp[j]*yp[j]+yp[j]*vy[j]; //  Producto escalar.
        double cross_pro = xp[j]*vy[j]-yp[j]*vx[j];
        
        V    += G * masa[j] / sqrt_x2_mas_y2;
        Eint += 0.5* masa[j] * vx2_mas_vy2; 

        Pcm    += masa[j] * sqrt_vx2_mas_vy2; 
        mtotal += masa[j]; 
        Rcm    += masa[j] * sqrt(pow(xp[j], 2) + pow(yp[j], 2));
        Lint   =  Rcm * sqrt_vx2_mas_vy2; 
        ...
        for (int i = 0; i < j; i++)
        {
            ...
            Vc += G * masa[i] * masa[j] / sqrt_x2_menos_y2;
        }
    }

    Rcm = Rcm/mtotal; // Posicion del centro de masas.
    P   = Pcm; // Momento lineal total.
    E   = Eint - V - Vc;
    L   = (Rcm*Pcm) + Lint; // Momento angular total.
}
*/
...
{
    // Parametros.
    const double h_step  = 6*3600;// (6 horas) //12*3600; // Tamanio de paso. (12 horas)
    const long int Niter = 7.3e6; //3.65e6; // Numero de iteraciones. (5000 anios)  
    ...
    // Archivos de salida.
    ofstream of_posicion("posicion-100-cuerpos-2.dat", ios::out);
    // ofstream of_velocidad("velocidad.dat", ios::out);
    ofstream of_energia("energia-100-cuerpos-2.dat", ios::out);
    ofstream of_momentumLineal("momLineal-100-cuerpos-2.dat", ios::out);
    ofstream of_momentumAngular("momAngular-100-cuerpos-2.dat", ios::out);
