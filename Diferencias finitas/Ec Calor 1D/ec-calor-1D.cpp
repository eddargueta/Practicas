...
void condiciones_frontera(double *u, int N, double tiempo)
{
    u[0] = p_cond_frontera(tiempo);
    u[N] = q_cond_frontera(tiempo);
}
...
int main()
{
    ...
    int Nt = 500000; // Numero de iteraciones en el tiempo.
    double tiempo  = 0.0; // LLeva la cuenta del tiempo.

    ofstream of;
    of.open("solucion-1-2.dat", ios::out);
    ...
    condiciones_frontera(u, N, tiempo);
